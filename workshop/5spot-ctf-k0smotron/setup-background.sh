...
FIVESPOT_IMAGE="${FIVESPOT_IMAGE:-ghcr.io/finos/5-spot:pr-92}"
# deploy/ manifests (CRD + RBAC + provider Deployments) must MATCH the controller image —
# pin to the PR branch, NOT the image tag ("pr-92" is not a git ref). The timebased-binary
# branch (PR #92) carries #82 (v1beta1 CRD) + #87 (provider-ref schedule) + #88 (CAPI
# passthrough) + #92 (the spot-schedule provider binary in the image); v0.2.2 would reject
# the v1beta1 ScheduledMachine in assets/.
FIVESPOT_REF="${FIVESPOT_REF:-timebased-binary}"
...
# ---- 5-Spot child kubeconfig (in-cluster endpoint + skip-TLS-verify) --------
# 5-Spot runs as a pod INSIDE the kind mgmt cluster and must reach the HOSTED control
# plane to taint/drain the worker Node. Two problems with the stock kubeconfig:
#   1) its server is the externalAddress (a LAN IP) — fine for the remote worker, but a
#      kind pod reaching it is a hairpin; the in-cluster NodePort service is the natural
#      path.
#   2) the hosted k0s API serves a self-signed cert that 5-Spot's TLS (rustls) REJECTS
#      where curl/OpenSSL accept it — so the child client fails with "client error
#      (Connect)" and taints/drains never happen.
# Give 5-Spot a dedicated kubeconfig that targets the in-cluster NodePort service and
# sets insecure-skip-tls-verify (kube honors it via Config::accept_invalid_certs). This
# is a lab hosted CP and the mgmt->CP hop stays inside the cluster, so skipping verify
# is acceptable. The ScheduledMachine points at it via spec.kubeconfigSecretRef.
echo "==> Building 5-Spot child kubeconfig (in-cluster endpoint, skip-TLS-verify)"
...
if clusterctl get kubeconfig dev-cluster > "$HOME/dev-cluster-5spot.kubeconfig" 2>/dev/null; then
  ...
  KUBECONFIG=$HOME/dev-cluster-5spot.kubeconfig kubectl config unset "clusters.${CL}.certificate-authority-data" >/dev/null 2>&1 || true
  KUBECONFIG=$HOME/dev-cluster-5spot.kubeconfig kubectl config set-cluster "$CL" --server="$KMC_ENDPOINT" --insecure-skip-tls-verify=true >/dev/null 2>&1 || true
  ...
  echo "    5-Spot will reach the hosted control plane at ${KMC_ENDPOINT} (skip-TLS-verify)"
else
