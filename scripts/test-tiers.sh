...
static_repo() {
  ...
  local pins
  pins=$(grep -h 'CLUSTERCTL_VERSION=' scripts/5-spot-bootstrap.sh workshop/*/setup-background.sh .devcontainer/post-create.sh \
  ...
  pins=$(grep -h 'FIVESPOT_IMAGE=' workshop/*/setup-background.sh \
...
PYEOF
  ...
  grep -q "RemoteMachine" workshop/5spot-ctf-k0smotron/assets/flux/scheduledmachine.yaml \
    && ok "k0smotron flux overlay uses RemoteMachine" || bad "k0smotron flux overlay is NOT the k0smotron SM"
  # no stale paths
  grep -rq "5spot-ctf/assets" workshop/ && bad "stale '5spot-ctf/assets' path found" || ok "no stale scenario paths"
  # portability: no hardcoded /root (breaks non-root Medium/Hard users)
  grep -rqE '(^|[^$])/root/' workshop/*/setup-background.sh workshop/*/*/verify.sh workshop/*/step*/text.md 2>/dev/null \
    && bad "hardcoded /root path found (use \$HOME)" || ok "no hardcoded /root paths"
  # helm present in pre-bakes (Flux/CoCo bonuses need it)
  for s in workshop/*/setup-background.sh; do
...
static_leaderboard() {
  ...
  n=$(grep -l 'post_flag()' workshop/*/*/verify.sh | wc -l)
  total=$(ls workshop/*/*/verify.sh | wc -l)
  ...
  n=$(grep -rhoE 'FLAG\{[A-Z0-9_]+\}' workshop/*/*/verify.sh | sort -u | wc -l)
...
static_kind() {
  ...
  bash -n workshop/5spot-ctf-capd/setup-background.sh && ok "CAPD bring-up script syntax" || bad "CAPD script"
...
live_kind() {
  ...
  local t0=$SECONDS
  if bash workshop/5spot-ctf-capd/setup-background.sh; then
    ok "pre-bake completed in $((SECONDS-t0))s"
    # Flag 1 end-to-end: apply SM, wait for node Ready, run the real verifier
    kubectl --context kind-5spot-mgmt apply -f workshop/5spot-ctf-capd/assets/scheduledmachine-business-hours.yaml \
      && ok "ScheduledMachine applied" || bad "SM apply failed"
    local deadline=$((SECONDS+600))
    until bash workshop/5spot-ctf-capd/step1-deploy/verify.sh >/dev/null 2>&1; do
      [ $SECONDS -gt $deadline ] && break; sleep 15; done
    bash workshop/5spot-ctf-capd/step1-deploy/verify.sh && ok "FLAG 1 verifier passes live" || bad "Flag 1 never verified (timeout)"
    # Flag 2
    ...
    until bash workshop/5spot-ctf-capd/step3-drain/verify.sh >/dev/null 2>&1; do
      [ $SECONDS -gt $deadline ] && break; sleep 15; done
    bash workshop/5spot-ctf-capd/step3-drain/verify.sh && ok "FLAG 3 verifier passes (graceful drain)" || bad "Flag 3 failed"
  else
...
live_k0smotron() {
  ...
  if REMOTE_NODE_HOST="$REMOTE_NODE_HOST" bash workshop/5spot-ctf-k0smotron/setup-background.sh; then
    ...
    kubectl --context kind-5spot-mgmt apply -f "$HOME/5spot-workshop/scheduledmachine-k0smotron.yaml" 2>/dev/null \
      || kubectl --context kind-5spot-mgmt apply -f workshop/5spot-ctf-k0smotron/assets/scheduledmachine-k0smotron.yaml
    local deadline=$((SECONDS+900))
    until bash workshop/5spot-ctf-k0smotron/step1-deploy/verify.sh >/dev/null 2>&1; do
      [ $SECONDS -gt $deadline ] && break; sleep 20; done
    bash workshop/5spot-ctf-k0smotron/step1-deploy/verify.sh && ok "FLAG 1 (remote worker) live" || bad "remote worker never Ready — check RemoteMachine fields vs your k0smotron release"
  else
